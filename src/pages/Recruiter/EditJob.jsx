    ...
    const fetchJobData = async () => {
      if (id) {
        ...
        if (jobData) {
          ...
          setLevel(jobData.level || "Entry Level");
          setJobType(jobData.jobType || "white");
          ...
          // Populate advanced values if present
          setVacancies(jobData.vacancies !== null && jobData.vacancies !== undefined ? String(jobData.vacancies) : "");
          setImmediateJoining(jobData.immediateJoining || false);
          setJoiningWithin(jobData.joiningWithin || "15 Days");
          setExperienceOption(jobData.experienceOption || "Fresher");
          setMinExperience(jobData.minExperience !== null && jobData.minExperience !== undefined ? String(jobData.minExperience) : "");
          setMaxExperience(jobData.maxExperience !== null && jobData.maxExperience !== undefined ? String(jobData.maxExperience) : "");
          setMinAge(jobData.minAge !== null && jobData.minAge !== undefined ? String(jobData.minAge) : "");
          setMaxAge(jobData.maxAge !== null && jobData.maxAge !== undefined ? String(jobData.maxAge) : "");
          setGenderPreference(jobData.genderPreference || "Any");

          if (jobData.educationRequirements && Array.isArray(jobData.educationRequirements)) {
            setEducationRequirements(jobData.educationRequirements);
          } else if (jobData.qualification) {
            setEducationRequirements([{ qualification: jobData.qualification, specializations: [] }]);
          }

          setLanguages(jobData.languages || []);
          setRequiredDocuments(jobData.requiredDocuments || []);
          
          if (jobData.shiftDetails) {
            setShiftDetails({
              shiftType: jobData.shiftDetails.shiftType || "General Shift",
              shiftTiming: jobData.shiftDetails.shiftTiming || "",
              weeklyOff: jobData.shiftDetails.weeklyOff || "Sunday",
              workingDays: jobData.shiftDetails.workingDays || "6"
            });
          }

          if (jobData.salaryBreakdown) {
            setSalaryBreakdown({
              inHand: jobData.salaryBreakdown.inHand || "",
              gross: jobData.salaryBreakdown.gross || "",
              ctc: jobData.salaryBreakdown.ctc || String(jobData.salary || ""),
              variablePay: jobData.salaryBreakdown.variablePay || "",
              incentives: jobData.salaryBreakdown.incentives || "",
              attendanceBonus: jobData.salaryBreakdown.attendanceBonus || "",
              productionBonus: jobData.salaryBreakdown.productionBonus || "",
              otAvailable: jobData.salaryBreakdown.otAvailable || "No",
              otRate: jobData.salaryBreakdown.otRate || ""
            });
          } else {
            setSalaryBreakdown({
              inHand: "",
              gross: "",
              ctc: jobData.salary ? String(jobData.salary) : "",
              variablePay: "",
              incentives: "",
              attendanceBonus: "",
              productionBonus: "",
          ...
          if (jobData.benefits) {
            setBenefits({
              food: jobData.benefits.food || [],
              transportation: jobData.benefits.transportation || [],
              uniform: jobData.benefits.uniform || [],
              incentives: jobData.benefits.incentives || [],
              overtime: jobData.benefits.overtime || [],
              accommodation: jobData.benefits.accommodation || [],
              healthcare: jobData.benefits.healthcare || [],
              other: jobData.benefits.other || []
            });
          }

          if (jobData.workLocationDetails) {
            setWorkLocationDetails({
              plantName: jobData.workLocationDetails.plantName || "",
              unitName: jobData.workLocationDetails.unitName || "",
              address: jobData.workLocationDetails.address || "",
              pinCode: jobData.workLocationDetails.pinCode || "",
              latitude: jobData.workLocationDetails.latitude || "",
              longitude: jobData.workLocationDetails.longitude || ""
            });
          }

          if (jobData.hrContact) {
            setHrContact({
              name: jobData.hrContact.name || "",
              mobile: jobData.hrContact.mobile || "",
              alternateMobile: jobData.hrContact.alternateMobile || "",
              email: jobData.hrContact.email || "",
              whatsapp: jobData.hrContact.whatsapp || ""
            });
          } else {
            setHrContact({
              name: jobData.companyDetails?.hrName || "",
              mobile: jobData.companyDetails?.hrPhone || "",
              alternateMobile: "",
          ...
          if (jobData.interviewProcess) {
            setInterviewProcess({
              type: jobData.interviewProcess.type || "Walk-In Interview",
              date: jobData.interviewProcess.date || "",
              time: jobData.interviewProcess.time || "",
              address: jobData.interviewProcess.address || "",
              link: jobData.interviewProcess.link || ""
            });
          }
    ...
    <div className="bg-gray-50 py-8 px-4 sm:px-6 lg:px-8 min-h-screen">
      <div className="max-w-5xl mx-auto">
        <div className="bg-white shadow-xl rounded-2xl overflow-hidden border border-gray-100">
          ...
          {/* Header */}
          <div className="bg-legpro-primary px-8 py-6 text-white flex justify-between items-center">
            <div>
              <h2 className="text-2xl font-bold flex items-center gap-2">
                <Briefcase className="h-6 w-6 text-blue-200" />
                Edit Job Details & ATS Config
              </h2>
              <p className="text-blue-100 text-sm mt-1">
                Refine granular requirements, salary details, and custom screening questions.
              </p>
            </div>
            <Link
              to="/dashboard/manage-jobs"
              className="border border-white/40 px-4 py-2 rounded-lg bg-white text-rose-600 font-bold hover:bg-rose-50 transition-all text-sm"
            >
              Cancel
            </Link>
          </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-slate-50 p-6 rounded-2xl border border-gray-200">
                    ...
                    <div className="md:col-span-2">
                      ...
                      <input
                        type="text"
                        placeholder="Complete office/venue address for walk-in"
